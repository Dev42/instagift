...
class PedidosController {
    ...
    public function insertAction(Pedidos $pedido) {
        ...
        $strQuery = "INSERT INTO " . $pedido->tableName() . " (`" . $fields . "`) VALUES('" . $values . "');";
    ...
    public function editAction(Pedidos $pedido){
        ...
        if ($pedido->getPaymode() != "" && $pedido->getPrdId() && $pedido->getCliId() != "" && $pedido->getQuantidade()){
            ...
            $sqlQuery = "UPDATE `fotu_net_br`.`".$pedido->tableName()."` SET $setQuery WHERE `ped_10_id` = ". $pedido->getId();
    ...
    public function deleteAction(Pedidos $pedido){
        ...
        if ($pedido->getId() != "") {
            ...
            $sqlQuery = "DELETE FROM `fotu_net_br`.`".$pedido->tableName()."` WHERE `ped_10_id` = ". $pedido->getId();
            mysql_query($sqlQuery);
