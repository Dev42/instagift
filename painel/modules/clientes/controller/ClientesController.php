...
class ClientesController {
    ...
    public function insertAction(Clientes $cliente) {
        ...
        if ($cliente->getNome() != "" && $cliente->getUserId() != "") {
            ...
            $strQuery = "INSERT INTO `fotu_net_br`.`" . $cliente->tableName() . "` (`" . $fields . "`) VALUES('" . $values . "');";
    ...
    public function editAction(Clientes $cliente){
        ...
        if ($cliente->getNome() != "" && $cliente->getUserId() != "") {
            ...
            $sqlQuery = "UPDATE `fotu_net_br`.`".$cliente->tableName()."` SET $setQuery WHERE `cli_10_id` = ". $cliente->getId();
    ...
    public function countChilds(Clientes $cli, $from = "contato"){
        ...
        $sqlQuery = "SELECT COUNT(*) as total FROM `fotu_net_br`.`".$from."` WHERE `cli_10_id` = ". $cli->getId();
    ...
    public function deleteAction(User $user){
        ...
        if ($user->getId() != "") {
            ...
            $sqlQuery = "DELETE FROM `fotu_net_br`.`".$user->tableName()."` WHERE `cli_10_id` = ". $user->getId();
            mysql_query($sqlQuery);
