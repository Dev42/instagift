...
class ProdutoInfoController {
    ...
    public function insertAction(ProdutoInfo $produto) {
        ...
        if ($produto->getNome() != "" && $produto->getDesc() != "" && $produto->getValor() != "" && $produto->getNrFotos() != "" && $produto->getPeso() != ""){
            ...
            $strQuery = "INSERT INTO `fotu_net_br`.`" . $produto->tableName() . "` (`" . $fields . "`) VALUES('" . $values . "');";
    ...
    public function editAction(ProdutoInfo $produto){
        ...
        if ($produto->getNome() != "" && $produto->getDesc() != "" && $produto->getValor() != "" && $produto->getNrFotos() != "" && $produto->getPeso() != ""){
            ...
            $sqlQuery = "UPDATE `fotu_net_br`.`".$produto->tableName()."` SET $setQuery WHERE `produto_info_10_id` = ". $produto->getId();
    ...
    public function deleteByProdutoAction($produtoId){
        ...
        if ($produtoId != "") {
            ...
            $sqlQuery = "DELETE FROM `fotu_net_br`.`produto_info` WHERE `produto_10_id` = ". $produtoId;
            mysql_query($sqlQuery);
    ...
    public function deleteAction(ProdutoInfo $produto){
        ...
        if ($produto->getId() != "") {
            ...
            $sqlQuery = "DELETE FROM `fotu_net_br`.`".$produto->tableName()."` WHERE `produto_info_10_id` = ". $produto->getId();
            mysql_query($sqlQuery);
